#!/usr/bin/env python3
"""CINEMA WATCH - 한국 영화관 좌석 / 예매 실시간 감시 콘솔.

    python cinema_watch.py                              대화형
    python cinema_watch.py --radar --cinema 월드타워 --once   헤드리스

롯데시네마 · 메가박스 · CGV 의 공개 조회 API 만 씁니다 (로그인·예매·결제 없음).
방향키로 영화관 -> 지역 -> 지점 -> 영화 -> 요일 -> 상영관 -> 좌석을 골라 감시 조건을
만들고, 조건이 충족되면(원하는 자리가 비거나 예매가 열리면) 알립니다.
...
ERRORLOG_FILE = os.path.join(APP_DIR, "errors.log")
...
DEFAULT_SETTINGS = {
    ...
    "seat_width": 0,       # 좌석표 최대 폭 (0 = 터미널 폭을 그대로)
    "last_brand": "",      # 마지막에 고른 브랜드 (다음 실행 때 커서를 여기에)
    "sweet_depth": 0.62,   # 명당 기준 깊이 (0=맨앞, 1=맨뒤)
    "notify": {},          # 알림 기본값 {interval, sound, open_browser, webhook, telegram}
}
...
class GoBack(Exception):
    """마법사에서 한 단계만 뒤로. m(BackToMenu)은 메인까지 나가지만 이건 직전 단계로."""
...
UI = {"ansi": True, "unicode": True, "tty": True, "width": 74, "term": 74}
...
def setup_terminal() -> None:
    ...
    try:                                      # 박스문자 출력 가능한지 확인
        ...
        UI["unicode"] = False
    # 박스·메뉴는 100칸까지만 쓴다(너무 넓으면 눈이 좌우로 흔들린다).
    # 좌석표는 넓을수록 잘 읽히므로 터미널 폭을 그대로 쓴다.
    cols = shutil.get_terminal_size((80, 25)).columns - 2
    UI["width"] = max(64, min(cols, 100))
    UI["term"] = max(64, cols)
...
def trim(text: str, width: int) -> str:
    """표시 폭에 맞춰 자른다. ANSI 코드는 폭을 차지하지 않으므로 건너뛰며 세고,
    자른 끝에 RESET 을 붙여 색이 다음 줄로 새지 않게 한다.
    """
...
def read_key_nowait() -> str:
    """눌린 키가 있으면 읽고, 없으면 빈 문자열. 감시 화면이 멈추지 않게 쓴다."""
    if not UI["tty"]:
        return ""
    try:
        if os.name == "nt":
            import msvcrt
            return read_key() if msvcrt.kbhit() else ""
        import select
...
class Chooser:
    ...
    def __init__(self, title: str, items: list, labeler, crumbs: list[str] | None = None,
                 ...
                 preview_label: str = "미리듣기", can_go_back: bool = True):
        self.title = title
        self.items = items
        ...
        try:
            while True:
                ...
                if key == "up":
                    ...
                    # 검색 중이면 검색어만 지우고, 아니면 한 단계 뒤로 나간다.
                    if self.filter:
                        self.filter = ""
                        self.cursor = self.top = 0
...
class Show:
    ...
    def parse(d: dict) -> "Show":
        # 상영관 이름은 "21관" 처럼 번호만 온다. 특별관 이름은 따로 있어서
        # (ScreenDivisionNameKR = 수퍼플렉스·광음LED·샤롯데 …) 붙여준다.
        name = (d.get("ScreenNameKR") or "").strip()
        div = (d.get("ScreenDivisionNameKR") or "").strip()
        if div and div != "일반" and div not in name:
            name = f"{name} {div}"
        return Show(
...
# 롯데 좌석 종류. `SeatBlockSet`("N,N,...,Y,N")에서 Y 의 자리가 종류를 가리킨다.
# 예매 화면 번들(TicketingPersonSeat.bundle.js)의 switch 문을 그대로 옮긴 것으로,
# 21관에서 빈백 26 · 소파베드 36 · 스위트 36 · 스탠다드 164 · 스튜디오 30 으로 대조 확인했다.
LOTTE_SEAT_KIND = {
    0: "시네커플석", 1: "시네패밀리석", 2: "슈퍼바이브석", 3: "장애인석",
    # 4·5·9 는 번들에서 이름을 갈아끼우지 않아 기본값 그대로 = 일반석이다.
    4: "일반석", 5: "일반석", 9: "일반석",
    6: "발받침석", 7: "시네커플 발받침석", 8: "장애인 발받침석",
    10: "특별요금석", 11: "시네패밀리 발받침석", 12: "슈퍼바이브 발받침석",
    13: "씨네비즈", 14: "리클라이너석", 15: "빈백", 16: "소파베드",
    17: "스위트(리클라이너)", 18: "스튜디오(부스)", 19: "스탠다드(리클라이너)",
    20: "프라이빗(2인)", 21: "프라이빗(4인)", 22: "소파석",
}
...
class SeatMap:
    ...
    zone: dict[str, str] = field(default_factory=dict)                # 라벨 -> 구역 이름
    total: int = 0
    ...
    def parse(d: dict) -> "SeatMap":
        ...
        for s in (d.get("Seats") or {}).get("Items", []):
            ...
            # 좌석 종류는 SeatBlockSet 의 Y 위치로 정해진다(예매 화면과 같은 규칙).
            blocks = str(s.get("SeatBlockSet") or "").replace(",", "")
            sm.zone[label] = LOTTE_SEAT_KIND.get(blocks.find("Y"), "일반석")
        for row in sm.grid:
            sm.grid[row].sort()
...
def sweet_depth() -> float:
    """명당 기준 깊이. 0 이면 맨 앞줄, 1 이면 맨 뒷줄. 설정에서 옮길 수 있다."""
    try:
        return min(max(float(SETTINGS.get("sweet_depth", 0.62)), 0.0), 1.0)
    except (TypeError, ValueError):
        return SWEET_DEPTH
...
def seat_area_width() -> int:
    """좌석표에 쓸 수 있는 폭. 설정값이 0 이면 터미널 폭을 그대로 쓴다.

    메뉴·상자는 UI["width"](최대 100)를 쓰지만 좌석표는 넓을수록 읽기 좋다.
    """
    try:
        fixed = int(SETTINGS.get("seat_width", 0) or 0)
    except (TypeError, ValueError):
        fixed = 0
    return max(48, fixed if fixed else UI["term"])


def seatmap_lines(smap: SeatMap, sweet: set[str] | None = None,
                  width: int | None = None) -> list[str]:
    """좌석 배치를 실제 좌표대로 그린다 (통로는 빈칸). sweet 를 주면 명당을 표시.

    width 를 주면 그 폭에 맞춘다(상자 안에 넣을 때). 없으면 터미널 폭을 다 쓴다.
    """
    if not smap.rows:
        return ["(좌석 정보 없음)"]
    avail_w = width or seat_area_width()
    ...
    if use_coord:
        ...
        if max(col_slot.values()) + 1 > avail_w - 8:
    ...
    # 자리가 남으면 좌석을 두 칸씩 벌려 그린다. 한 칸씩 붙여 놓으면 촘촘해서
    # 어느 자리가 어느 자리인지 눈이 따라가기 힘들다.
    gap = 2 if (width * 2 + 12) <= avail_w else 1

    def stretch(cells: list[str]) -> str:
        return (" " * (gap - 1)).join(cells) if gap > 1 else "".join(cells)

    # 눈금은 늘린 뒤의 화면 위치에 맞춰 만든다. 먼저 만들어 놓고 같이 늘리면
    # 10 이 "1 0" 으로 갈라진다.
    span = width * gap - (gap - 1)
    ruler = [" "] * span
    for slot in sorted(by_slot):
        ...
        if no != 1 and no % 5:
            continue
        text = str(no)
        start = max(0, slot * gap - len(text) + 1)
        if start + len(text) > span:
            continue
        if all(ruler[i] == " " for i in range(start, start + len(text))):
            for i, ch in enumerate(text):
                ruler[start + i] = ch

    # 구역이 둘 이상이면 남은 자리를 구역별 색으로 나눈다(나간 자리는 그대로 흐리게).
    zones = sorted({z for lab, z in smap.zone.items() if z and lab in
                    (smap.available | smap.taken)})
    zone_color = {z: c for z, c in zip(zones, (C.BGREEN, C.BCYAN, C.MAGENTA,
                                               C.BLUE, C.BYELLOW, C.WHITE))}
    # 명당과는 겹치지 않는다 - 명당은 글자(◆), 구역은 색으로 나누기 때문.
    use_zone = len(zones) > 1
    # 색이 여섯 가지를 넘으면 알아보기 어려우니 글자도 함께 달리한다.
    # 색을 못 쓰는 터미널에서도 이 글자 구분은 살아남는다.
    zone_mark = {}
    if len(zones) > 6:
        shapes = [glyph(g, a) for g, a in
                  (("■", "O"), ("▣", "@"), ("▤", "#"), ("▦", "%"),
                   ("▩", "&"), ("▥", "="), ("▧", "+"), ("▨", "*"))]
        zone_mark = {z: shapes[i % len(shapes)] for i, z in enumerate(zones)}

    pad_left = "     "
    out = []
    dash = glyph("─", "-")
    if span >= 12:                        # 좌석 영역 폭에 정확히 맞춘다
        rest = span - 8
        out.append(f"{C.DIM}{pad_left}{dash * (rest // 2)} SCREEN "
                   f"{dash * (rest - rest // 2)}{C.RESET}")
    else:
        out.append(f"{C.DIM}{pad_left}{'SCREEN'.center(span)}{C.RESET}")
    ...
    for row in smap.rows:
        ...
        for col, ok in smap.grid[row]:
            ...
            if not ok:
                ...
                # 글자로 명당 여부, 색으로 구역을 나타내 둘을 한 화면에 담는다.
                zname = smap.zone.get(lab, "")
                mark = (glyph("◆", "@") if sweet and lab in sweet
                        else zone_mark.get(zname, glyph("■", "O")))
                if use_zone:
                    color = zone_color.get(zname, C.BGREEN)
                elif sweet and lab in sweet:
                    color = C.BYELLOW
                else:
                    color = C.BGREEN
                cells[slot_of(lab)] = f"{color}{mark}{C.RESET}"
        free = sum(1 for _, ok in smap.grid[row] if ok)
        tail = f"  {C.BGREEN}{free:>2}{C.RESET}" if free else f"  {C.DIM} -{C.RESET}"
        # 다섯 줄마다 행 이름을 밝게 -- 세로로 훑을 때 위치를 잃지 않게.
        label = f"{C.BCYAN}{C.BOLD}{row:<2}{C.RESET}" if shown % 5 == 0 else \
                f"{C.BOLD}{row:<2}{C.RESET}"
        out.append(f"{label}   {stretch(cells)}{tail}")
        shown += 1

    if use_zone:
        counts = Counter(smap.zone.get(lab, "") for lab in smap.available)
        parts = [f"{zone_color.get(z, C.BGREEN)}"
                 f"{zone_mark.get(z, glyph('■', 'O'))}{C.RESET}"
                 f"{C.DIM} {z} {counts[z]}{C.RESET}"
                 for z in zones if counts.get(z)]
        parts.append(f"{C.DIM}{glyph('·', 'X')} 불가 {len(smap.taken)}{C.RESET}")
        # 한 줄에 몰면 상자 안에서 잘리고, 잘리는 순간 색까지 통째로 날아간다.
        # (trim 이 ANSI 를 걷어내기 때문) 그래서 폭에 맞춰 미리 나눠 둔다.
        limit = avail_w - 10
        line = "   "
        for part in parts:
            if dwidth(line) + dwidth(part) + 3 > limit and line.strip():
        ...
        if line.strip():
            out.append(line.rstrip())
        if sweet:
            out.append(f"{C.DIM}   {glyph('◆', '@')} 표시는 명당 "
                       f"{len(sweet & smap.available)}석 (색은 좌석 종류){C.RESET}")
    elif sweet:
        out.append(f"{C.DIM}   {glyph('◆', '@')} 명당 {len(sweet & smap.available)}석 가능   "
                   f"{glyph('■', 'O')} 그 외 가능   {glyph('·', 'X')} 불가"
                   f"   (숫자=좌석번호){C.RESET}")
    else:
        out.append(f"{C.DIM}   {glyph('■', 'O')} 예매가능 {len(smap.available)}석   "
...
class Watch:
    ...
    seat_kinds: list[str] = field(default_factory=list)   # 비어 있으면 종류를 안 가림
    interval: int = 20
    sound: bool = True
    webhook: str = ""
    ...
    def fields(self) -> list[tuple[str, str]]:
        ...
        if self.mode == "seat":
            rows += [("요일", f"{wd}  (오늘부터 {self.days_ahead}일)"),
                     ...
                     ("좌석", self.seat_text())]
            if self.seat_kinds:
                rows.append(("좌석종류", ", ".join(self.seat_kinds)))
...
def write_profiles(profiles: list[dict]) -> bool:
    try:
        with open(PROFILE_FILE, "w", encoding="utf-8") as fh:
            json.dump({"profiles": profiles}, fh, ensure_ascii=False, indent=2)
        return True
    except Exception as exc:
        print(f"   {C.YELLOW}설정 파일을 쓰지 못했습니다: {exc}{C.RESET}")
        return False


def delete_profile(name: str) -> bool:
    return write_profiles([p for p in load_profiles()
...
def rename_profile(old: str, new: str) -> bool:
    rows = load_profiles()
    for p in rows:
        if p.get("profile_name") == old:
            p["profile_name"] = new
    return write_profiles(rows)
...
class Favorites:
    ...
    def __init__(self) -> None:
        ...
        self.movies: list[str] = []                 # 늘 쫓는 작품 (브랜드 공통)
        self.load()
        ...
        try:
            ...
            if isinstance(raw, list):               # 예전 형식(롯데 전용) 이전
                ...
                self.screens = {str(k): list(v)
                                for k, v in (data.get("screens") or {}).items()}
            self.movies = [str(x) for x in (data.get("movies") or [])]
        except Exception:
            pass
        ...
        try:
            with open(FAVORITE_FILE, "w", encoding="utf-8") as fh:
                json.dump({"cinemas": self.cinemas, "screens": self.screens,
                           "movies": self.movies}, fh, ensure_ascii=False, indent=2)
        except Exception as exc:
...
class Provider:
    ...
    @classmethod
    def booking_url(cls, show: "Show | None" = None) -> str:
        """예매 페이지 주소. 회차를 주면 되도록 그 회차 가까이로 보낸다."""
        return cls.ticketing_url
...
class LotteProvider(Provider):
    ...
    예매 페이지는 화면을 눌러 들어가면 URL 이 그대로지만, 외부 유입용
    link_* 파라미터는 받는다(네이버 지도가 이 방식으로 넘긴다).
    """
    code = "lotte"
    name = "롯데시네마"
    color_name = "BRED"
    ...
    @classmethod
    def booking_url(cls, show: "Show | None" = None) -> str:
        # 확인한 파라미터: link_screenId · link_cinemaCode · link_movieCd
        # · link_date · link_time. 상영표가 주는 값이 그대로 들어맞는다.
        # link_channelCode 가 없으면 날짜를 무시하고 오늘로 열린다(실측).
        if show and show.screen_id and show.movie_code:
            q = urllib.parse.urlencode({
                "link_screenId": show.screen_id, "link_cinemaCode": show.cinema_id,
                "link_movieCd": show.movie_code, "link_date": show.play_date,
                "link_time": show.start, "link_channelCode": "naver"})
            return f"{cls.ticketing_url}?{q}"
        return cls.ticketing_url
...
# 메가박스는 좌석 등급을 코드로만 준다. 화면에 쓸 이름으로 바꿔둔다.
MEGABOX_SEAT_CLASS = {
    "GERN_CLS": "일반석", "RECLINE_CLS": "리클라이너", "DISABLED_CLS": "장애인석",
    "COUPLE_CLS": "커플석", "SWEET_CLS": "스위트", "PRIME_CLS": "프라임",
}
...
class MegaboxProvider(Provider):
    ...
    ticketing_url = BASE + "/booking/timetable"

    @classmethod
    def booking_url(cls, show: "Show | None" = None) -> str:
        # 회차 단위로 바로 열린다 - 좌석선택 화면이 URL 파라미터를 받는다.
        if show and show.play_seq:
            return (f"{cls.BASE}/booking/seat?playSchdlNo={show.play_seq}"
                    f"&brchNo={show.cinema_id}")
        return cls.ticketing_url
    ...
    def seats(self, show: Show) -> SeatMap:
        ...
        for s in data.get("seatListSD01") or []:
            ...
            if x is not None and y is not None:
                ...
                sm.coord[label] = (int(x), int(y))
            cls = str(s.get("seatClassCd") or "")
            if cls:
                sm.zone[label] = MEGABOX_SEAT_CLASS.get(cls, cls.replace("_CLS", ""))
        for row in sm.grid:
            sm.grid[row].sort()
        sm.rows = sorted(sm.grid)
        sm.total = len(sm.available) + len(sm.taken)
...
class CgvProvider(Provider):
    ...
    ticketing_url = BASE + "/cnm/movieBook"

    @classmethod
    def booking_url(cls, show: "Show | None" = None) -> str:
        # 회차까지 담긴다. 상영표가 주는 값이 파라미터와 그대로 맞는다.
        if show and show.screen_id and show.movie_code:
            # 상영표는 극장명을 "CGV 천호" 로 주는데 URL 파라미터는 "천호" 다.
            name = re.sub(r"^CGV\s*", "", show.cinema_name)
            q = urllib.parse.urlencode({
                "movNo": show.movie_code, "scnYmd": show.play_date.replace("-", ""),
                "siteNo": show.cinema_id, "siteNm": name,
                "scnsNo": show.screen_id, "scnSseq": show.play_seq})
            return f"{cls.BASE}/cnm/movieBook/movie?{q}"
        return cls.ticketing_url
        ...
        for item in (data.get("data") or {}).get("items") or []:
            for s in item.get("seats") or []:
                ...
                try:
                    ...
                    pass
                # CGV 만 구역 이름을 그대로 준다 ("일반", "Light존" 등)
                zname = str(s.get("szoneKindNm") or s.get("stkndNm") or "").strip()
                if zname:
                    sm.zone[label] = zname
        for row in sm.grid:
            sm.grid[row].sort()
...
class ShowTracker:
    ...
    def _append_log(self, show: Show, delta: int) -> None:
        ...
        row = {"t": datetime.now().isoformat(timespec="seconds"),
               "brand": self.cfg.brand,        # 지점 번호만으로는 브랜드가 안 갈린다
               "cinema": show.cinema_id, "cinema_name": show.cinema_name,
...
def limit_to_kinds(cfg: Watch, smap: SeatMap) -> SeatMap:
    """고른 좌석 종류만 남긴 좌석표를 돌려준다. 종류를 안 골랐으면 그대로.

    세 브랜드 모두 좌석 종류를 주므로 '리클라이너만', '스위트만' 같은 조건이 된다.
    걸러낸 자리는 나간 자리처럼 다뤄 이후 판정(연속석·명당)이 그대로 통한다.
    """
    if not cfg.seat_kinds or not smap.zone:
        return smap
    want = set(cfg.seat_kinds)
    out = SeatMap(rows=smap.rows, coord=smap.coord, zone=smap.zone, total=smap.total)
    for row in smap.rows:
        for col, ok in smap.grid[row]:
            lab = f"{row}{col}"
            keep = ok and smap.zone.get(lab) in want
            (out.available if keep else out.taken).add(lab)
            out.grid.setdefault(row, []).append((col, keep))
    return out


def evaluate_seats(cfg: Watch, smap: SeatMap) -> tuple[bool, str]:
    smap = limit_to_kinds(cfg, smap)
...
class Notifier:
    ...
    def fire(self, headline: str, detail: str, show: "Show | None" = None) -> None:
        if self.sound:
            threading.Thread(target=self._beep, daemon=True).start()
        # 회차를 알면 그 회차 예매 화면으로 바로 보낸다(브랜드가 지원하는 만큼).
        cls = PROVIDERS.get(self.brand)
        link = cls.booking_url(show) if cls else TICKETING_URL
        text = f"{headline}\n{detail}\n{link}"
        if self.webhook:
...
def wiz_cinema(cat: Catalog, crumbs: list[str], fav: Favorites | None = None) -> Cinema:
    ...
    # 지점이 100곳을 넘어 지역부터 고르는 게 번거로울 때가 많다. 전체에서 바로 찾는 길.
    regions.insert(0, ("*", f"전체 지점에서 찾기 ({len(cat.cinemas())}곳)"))
    if favs:
        regions.insert(0, ("★", f"즐겨찾는 지점 ({len(favs)}곳)"))
    region = Chooser("지역 선택", regions,
                     lambda r: starred(r[1], "" if r[0] in ("★", "*")
                                       else f"{len(cat.by_region(r[0]))}개 지점",
                                       r[0] == "★", right_width=12),
                     crumbs, hint="'/' 로 지점 이름을 바로 검색할 수 있습니다").run()

    if region[0] == "★":
        cinemas = favs
    elif region[0] == "*":
        cinemas = fav.sort_cinemas(list(cat.cinemas()))
    else:
        cinemas = fav.sort_cinemas(cat.by_region(region[0]))
...
def wiz_movie(shows: list[Show], crumbs: list[str],
              fav: Favorites | None = None) -> str:
    counts: dict[str, int] = {}
    for s in shows:
    ...
    # 늘 쫓는 작품은 맨 위로 올린다. f 로 즐겨찾기를 켜고 끈다.
    if fav is not None:
        ranked.sort(key=lambda m: (m[0] not in fav.movies, -m[1]))

    def toggle(item) -> None:
        if fav is None:
            return
        if item[0] in fav.movies:
            fav.movies.remove(item[0])
        else:
            fav.movies.append(item[0])
        fav.save()

    picked = Chooser("상영 영화 선택", ranked,
                     lambda m: starred(m[0], f"{m[1]:>3}회차",
                                       fav is not None and m[0] in fav.movies,
                                       right_width=10), crumbs,
                     hint="'/' 검색 · f 로 즐겨찾기" if fav is not None else "",
                     preview=("f", toggle) if fav is not None else None,
                     preview_label="즐겨찾기",
...
def show_seatmap_preview(cat: Catalog, cfg: Watch, shows: list[Show],
    ...
    try:
        ...
        for line in seatmap_lines(smap, spots, width=UI["width"] - 6):
            print(box_row(line))
        if spots:
            scores = sweet_scores(smap)
...
def notify_defaults() -> dict:
    saved = SETTINGS.get("notify")
    return dict(saved) if isinstance(saved, dict) else {}


def apply_notify_defaults(cfg: Watch) -> bool:
    """저장해 둔 알림 기본값을 그대로 얹는다. 쓸 값이 없으면 False."""
    d = notify_defaults()
    if not d:
        return False
    cfg.interval = int(d.get("interval", cfg.interval))
    cfg.sound = bool(d.get("sound", cfg.sound))
    cfg.open_browser = bool(d.get("open_browser", cfg.open_browser))
    cfg.webhook = str(d.get("webhook", ""))
    cfg.telegram = dict(d.get("telegram") or {})
    cfg.repeat_alert = bool(d.get("repeat_alert", cfg.repeat_alert))
    return True


def notify_summary(d: dict) -> str:
    bits = [f"{d.get('interval', 20)}초"]
    bits.append("소리" if d.get("sound", True) else "무음")
    if d.get("webhook"):
        bits.append("웹훅")
    if (d.get("telegram") or {}).get("token"):
        bits.append("텔레그램")
...
def wiz_notify(cfg: Watch, crumbs: list[str]) -> None:
    # 매번 처음부터 답하지 않도록, 지난번 값을 그대로 쓰는 길을 먼저 준다.
    saved = notify_defaults()
    if saved:
        pick = Chooser("알림 설정", [
            ("same", "지난번 그대로", notify_summary(saved), C.BCYAN),
            ("edit", "다시 고르기", "주기 · 소리 · 웹훅 · 텔레그램", ""),
        ], lambda it: two_col(it[1], it[2], right_width=34, color=it[3]),
            crumbs).run()[0]
        if pick == "same":
            apply_notify_defaults(cfg)
            return
    ...
    # 다음 감시부터는 '지난번 그대로' 한 번으로 넘어갈 수 있게 남겨둔다.
    SETTINGS["notify"] = {"interval": cfg.interval, "sound": cfg.sound,
                          "open_browser": cfg.open_browser, "webhook": cfg.webhook,
                          "telegram": cfg.telegram, "repeat_alert": cfg.repeat_alert}
    save_settings()
...
def wiz_seat_kinds(cat: Catalog, cfg: Watch, shows: list[Show],
                   crumbs: list[str]) -> None:
    """좌석 종류를 고른다 (리클라이너만, 스위트만 …).

    종류는 상영관마다 다르므로 감시 대상 회차 하나의 좌석표를 실제로 받아
    거기 있는 종류만 보여준다. 두 가지 이상 있을 때만 묻는다.
    """
    if not cat.provider.supports_seats:
        return
    cands = [s for s in filter_shows(shows, cfg) if s.total_seats > 0]
    if not cands:
        return
    try:
        smap = cat.seats(cands[0])
    except (LotteApiError, Unsupported):
        return
    counts = Counter(z for z in smap.zone.values() if z)
    if len(counts) < 2:
        return

    kinds = [k for k, _ in counts.most_common()]
    rows = [("", f"전체 좌석 종류 ({len(kinds)}가지)", f"{smap.total}석", C.BCYAN)]
    rows += [(k, k, f"{counts[k]}석", "") for k in kinds]
    clear_screen()
    print("\n".join(header("좌석 종류", crumbs)))
    print(box_row(f"{C.DIM}{cands[0].screen_name} 기준입니다. 고른 종류에서만 자리를"
                  f" 찾습니다.{C.RESET}"))
    print(box_bottom())
    picked = Chooser("어떤 좌석을 노릴까요", rows,
                     lambda it: two_col(it[1], it[2], right_width=12, color=it[3]),
                     crumbs, multi=True,
                     hint="Space 로 여러 개 · 아무것도 안 고르면 전체").run()
    chosen = [p[0] for p in picked if p[0]]
    cfg.seat_kinds = chosen if len(chosen) < len(kinds) else []


def wizard_seat(cat: Catalog, fav: Favorites) -> Watch:
    cfg = Watch(mode="seat")
    base = ["좌석 감시"]
    state: dict = {"cinema": None, "shows": []}

    def step_cinema() -> None:
        state["cinema"] = wiz_cinema(cat, base, fav)
        apply_cinema(cfg, state["cinema"], cat)

    def step_range() -> None:
        ranges = [(7, "1주일"), (14, "2주일"), (30, "1개월"), (3, "3일")]
        cfg.days_ahead = Chooser(
            "며칠 앞까지 감시할까요", ranges,
            lambda r: f"{pad(r[1], 10)}{C.DIM}오늘부터 {r[0]}일{C.RESET}",
            base + [state["cinema"].name]).run()[0]
        clear_screen()
        print("\n".join(header(f"{state['cinema'].name} 상영표 수집",
                               base + [state["cinema"].name])))
        print(box_bottom())
        state["shows"] = cat.scan(state["cinema"], min(cfg.days_ahead, 14))

    def step_movie() -> None:
        cfg.movie = wiz_movie(state["shows"], base + [state["cinema"].name], fav)

    def crumbs_now() -> list[str]:
        return base + [state["cinema"].name, cfg.movie]

    def step_weekdays() -> None:
        cfg.weekdays = wiz_weekdays(crumbs_now())

    def step_screens() -> None:
        cfg.screens = wiz_screens(state["shows"], cfg.movie, crumbs_now(),
                                  state["cinema"], fav)

    def step_time() -> None:
        wiz_time_range(cfg, crumbs_now())

    def step_kinds() -> None:
        wiz_seat_kinds(cat, cfg, state["shows"], crumbs_now())

    # 배열 + 인덱스로 두면 Esc(GoBack) 를 만났을 때 한 단계만 물러날 수 있다.
    steps = [step_cinema, step_range, step_movie, step_weekdays,
             step_screens, step_time, step_kinds]
    i = 0
    while i < len(steps):
        try:
            steps[i]()
...
def narrow_shows(shows: list[Show], crumbs: list[str]) -> list[Show]:
    """영화 → 상영관 → 시간대 순으로 좁힌다. 고를 게 하나뿐이면 그 단계는 건너뛴다."""
    movies = sorted({s.movie_name for s in shows})
    if len(movies) > 1:
        rows = [("", f"전체 영화 ({len(movies)}편)", f"{len(shows)}회차", C.BCYAN)]
        rows += [(mv, mv, f"{sum(1 for s in shows if s.movie_name == mv)}회차", "")
                 for mv in movies]
        pick = Chooser("영화 선택", rows,
                       lambda it: two_col(it[1], it[2], right_width=12, color=it[3]),
                       crumbs, hint="/ 로 제목 검색").run()[0]
        if pick:
            shows = [s for s in shows if s.movie_name == pick]
            crumbs = crumbs + [trim(pick, 14)]

    screens = sorted({s.screen_name for s in shows})
    if len(screens) > 1:
        rows = [("", f"전체 상영관 ({len(screens)}개)", f"{len(shows)}회차", C.BCYAN)]
        rows += [(sc, sc, f"{sum(1 for s in shows if s.screen_name == sc)}회차", "")
                 for sc in screens]
        pick = Chooser("상영관 선택", rows,
                       lambda it: two_col(it[1], it[2], right_width=12, color=it[3]),
                       crumbs).run()[0]
        if pick:
            shows = [s for s in shows if s.screen_name == pick]
            crumbs = crumbs + [trim(pick, 12)]

    if len(shows) > 5:
        rows = []
        for key, label, lo, hi in TIME_BANDS:
            n = (len(shows) if not key
                 else sum(1 for s in shows if lo <= s.start < hi))
            if key and not n:
                continue
            rows.append((key, label, f"{n}회차", C.BCYAN if not key else ""))
        if len(rows) > 2:
            pick = Chooser("시간대", rows,
                           lambda it: two_col(it[1], it[2], right_width=12, color=it[3]),
                           crumbs).run()[0]
            if pick:
                lo, hi = next((b[2], b[3]) for b in TIME_BANDS if b[0] == pick)
                shows = [s for s in shows if lo <= s.start < hi]
    return shows
...
def browse_seats(cat: Catalog, fav: Favorites) -> None:
    ...
    while True:
        ...
        shows = narrow_shows(shows, crumbs + [day[5:]])
        if not shows:
            continue
        ...
        view_one_show(cat, picked, crumbs + [day[5:]], cinema, shows)
        if not ask_yes("다른 회차를 더 볼까요?", True):
            return


def view_one_show(cat: Catalog, show: Show, crumbs: list[str], cinema: Cinema,
                  siblings: list[Show] | None = None) -> None:
    """회차 하나의 좌석표를 그린다. r 새로고침 · ← → 옆 회차 · o 예매 페이지."""
    brand = cat.provider
    pool = siblings or [show]
    idx = next((i for i, s in enumerate(pool) if s.key == show.key), 0)
    while True:
        show = pool[idx]
        clear_screen()
        smap = None
        ...
        if smap is None:
            ...
            hints = [("r", "새로고침")]
            if len(pool) > 1:
                hints.append((glyph("←→", "</>"), f"회차 ({idx + 1}/{len(pool)})"))
            hints += [("o", "예매 페이지"), ("Enter", "돌아가기")]
            print(keyhint(*hints))
            key = read_key() if UI["tty"] else "enter"
            if key in ("left", "right") and len(pool) > 1:
                idx = (idx + (1 if key == "right" else -1)) % len(pool)
                continue
            if key in ("char:o", "char:O"):
                open_link(brand.booking_url(show))
                continue
        ...
        sweet = sweet_seats(smap, 0.2) if smap.coord else None
        print()
        ...
        if key in ("left", "right") and len(pool) > 1:
            ...
            continue
        if key in ("char:o", "char:O"):
            link = brand.booking_url(show)
            if not open_link(link):
                print(f"   {C.YELLOW}브라우저를 열지 못했습니다: {link}{C.RESET}")
                read_line(f"   {C.DIM}Enter 로 계속{C.RESET}")
            continue
        if key not in ("char:r", "char:R"):
            return
...
def wizard_open(cat: Catalog, fav: Favorites) -> Watch:
    ...
    cfg.movie = wiz_movie(shows, crumbs, fav)
...
class State:
    def __init__(self, cfg: Watch):
        ...
        self.next_at = 0.0
        self.calls = 0
        self.hits: deque[tuple[str, str]] = deque(maxlen=10)
        self.changes: deque[tuple[str, str]] = deque(maxlen=6)
        self.targets: list[str] = []          # 지금 감시 중인 회차 (l 로 펼쳐 본다)
        ...
        self.finished = False
        self.errors = 0
        self.paused = threading.Event()       # 켜져 있으면 워커가 멈춰 선다
        self.wake = threading.Event()         # 대기 중에도 즉시 한 번 더 확인
    ...
    def note(self, text: str) -> None:
        """화면에 알리고 errors.log 에도 남긴다.

        화면 표시는 두 줄뿐이라 다음 주기에 밀려 사라진다. 밤새 돌린 뒤
        '왜 아무것도 못 잡았는지' 를 되짚으려면 기록이 남아 있어야 한다.
        """
        with self.lock:
            self.notes.appendleft(text)
            self.errors += 1
        log_error(f"[{self.cfg.mode}] {ANSI_RE.sub('', text)}")


def idle_wait(st: "State", seconds: float) -> None:
    """다음 주기까지 기다린다. 기다리는 동안에도 일시정지·즉시확인·중지에 반응한다."""
    st.set(next_at=time.time() + max(seconds, 1.0))
    if st.wake.wait(max(seconds, 1.0)):        # r 을 누르면 곧장 깨어난다
        st.wake.clear()
    while st.paused.is_set() and not st.stop.is_set():
        st.set(phase="일시정지 (p 로 재개)", next_at=0.0)
        st.stop.wait(0.2)


def log_error(text: str) -> None:
    """실패를 errors.log 에 한 줄로 남긴다. 기록 자체가 실패해도 조용히 넘어간다."""
    try:
        with open(ERRORLOG_FILE, "a", encoding="utf-8") as fh:
...
def dashboard_lines(st: State) -> list[str]:
    ...
    with st.lock:
        ...
        horizon, events, errors = st.horizon, st.events, st.errors

    mode_name = {"seat": "좌석 감시", "open": "예매오픈 감시",
                 "radar": "오픈 패턴 관측"}.get(cfg.mode, cfg.mode)
    lines = [
        f"{ACTIVE['color']}{C.BOLD}{ACTIVE['name']}{C.RESET}  {C.DIM}v{VERSION}{C.RESET}   "
        f"{C.ON_BLUE}{C.BWHITE} {mode_name} {C.RESET}",
        "",
        box_top("감시 조건"),
    ]
    ...
    if errors:
        lines.append(box_row(f"{C.DIM}{pad('실패', 10)}{C.RESET}"
                             f"{C.YELLOW}{errors}건{C.RESET}"
                             f"{C.DIM}  {os.path.basename(ERRORLOG_FILE)} 에 기록{C.RESET}"))
    ...
    lines.append(box_bottom())
    hints = [("p", "일시정지" if not st.paused.is_set() else "재개"),
             ("r", "지금 확인"), ("+/-", "주기")]
    if cfg.mode == "seat":
        hints.append(("l", "대상 보기"))
    hints.append(("q", "중지"))
    lines.append(keyhint(*hints))
    return lines
...
def worker_seat(cat: Catalog, cfg: Watch, note: Notifier, st: State) -> None:
    ...
    while not st.stop.is_set():
        ...
        # 상영표 갱신 - 호출 한 번으로 그 날짜 전 회차의 잔여석을 받는다.
        ...
        # 확인할 회차 선별 - 취소표 감지 우선, 그다음 예정 시각이 된 순.
        now = time.monotonic()
        ...
        st.set(phase="좌석 확인 중", prog=(0, len(queue)), tiers=tiers,
               ...
               targets=[f"{s.short()}  잔여 {s.open_seats}/{s.total_seats}"
                        for s in sorted(live, key=Scheduler.show_dt)])
        checked = matched = 0
        ...
        idle_wait(st, cfg.interval - elapsed)
    ...
    while pending and not st.stop.is_set():
        ...
        for i, day in enumerate(list(pending), 1):
            ...
            note.fire(f"[예매 오픈] {cfg.movie} {day}",
                      ...
                      bookable[0] if bookable else None)
...
def show_targets(st: State) -> None:
    """지금 감시 중인 회차를 펼쳐 본다 (감시 화면에서 l).

    '23회차' 라는 숫자만 보여서, 조건을 잘못 걸어 엉뚱한 걸 보고 있어도 알 수 없었다.
    """
    with st.lock:
        rows = list(st.targets)
    show_cursor()
    clear_screen()
    print("\n".join(header(f"감시 중인 회차 ({len(rows)}개)", ["감시"])))
    if rows:
        room = max(UI["term"] // 40, 1)        # 넓으면 여러 줄을 한 화면에
        for i in range(0, min(len(rows), 40), room):
            print(box_row("   ".join(cell(r, 38) for r in rows[i:i + room])))
        if len(rows) > 40:
            print(box_row(f"{C.DIM}… 그 밖에 {len(rows) - 40}회차{C.RESET}"))
    else:
        print(box_row(f"{C.DIM}아직 대상이 정해지지 않았습니다"
                      f" (첫 주기를 도는 중){C.RESET}"))
    print(box_bottom())
    read_line(f"   {C.DIM}Enter 로 감시 화면으로{C.RESET}")
    hide_cursor()
    clear_screen()


def handle_monitor_key(st: State, cfg: Watch) -> bool:
    """감시 화면에서 누른 키를 처리한다. True 를 돌려주면 감시를 끝낸다.

    감시를 켜 둔 뒤 할 수 있는 일이 끄는 것뿐이라, 주기 하나 바꾸려 해도
    마법사를 처음부터 다시 걸어야 했다.
    """
    key = read_key_nowait()
    if not key:
        return False
    if key in ("char:q", "char:Q"):
        st.stop.set()
        st.wake.set()
        return True
    if key in ("char:l", "char:L"):
        show_targets(st)
    if key in ("char:p", "char:P"):
        if st.paused.is_set():
            st.paused.clear()
            st.wake.set()                      # 재개하면 기다리지 않고 바로 확인
        else:
            st.paused.set()
    elif key in ("char:r", "char:R"):
        st.paused.clear()
        st.wake.set()
    elif key in ("char:+", "char:=", "char:-", "char:_"):
        step = 5 if cfg.interval < 60 else 30
        if key in ("char:-", "char:_"):
            cfg.interval = max(5, cfg.interval - step)
        else:
            cfg.interval = min(3600, cfg.interval + step)
    return False
        ...
        while thread.is_alive():
            if UI["ansi"]:                     # 제자리 갱신 (초시계까지 실시간)
                ...
                if sig != last_sig:
                    ...
                    paint(dashboard_lines(st))
                time.sleep(1.0)
            if handle_monitor_key(st, cfg):    # q 를 누르면 감시를 끝낸다
                break
        paint(dashboard_lines(st))
...
def wizard_radar(cat: Catalog, fav: Favorites) -> None:
    ...
    movie = wiz_movie(shows, crumbs, fav)
    crumbs = crumbs + [movie]
...
def pick_brand() -> Provider:
    ...
    # 대개 같은 곳을 보므로 지난번에 고른 브랜드에 커서를 얹어 둔다.
    last = str(SETTINGS.get("last_brand", ""))
    start = next((i for i, cls in enumerate(items) if cls.code == last), 0)
    picker = Chooser("영화관 선택", items, label,
                     hint="브랜드마다 공개된 데이터의 깊이가 다릅니다")
    picker.cursor = start
    provider = picker.run()()
    ACTIVE.update(code=provider.code, color=provider.color(), name=provider.title)
    if SETTINGS.get("last_brand") != provider.code:
        SETTINGS["last_brand"] = provider.code
        save_settings()
    return provider


def find_cinema(cat: Catalog, needle: str) -> Cinema | None:
    """지점 번호 또는 이름 일부로 지점을 찾는다."""
    needle = needle.strip()
    rows = cat.cinemas()
    for c in rows:                             # 번호가 정확히 맞는 것부터
        if str(c.cinema_id) == needle or str(c.key) == needle:
            return c
    hits = [c for c in rows if needle and needle in c.name]
    return hits[0] if len(hits) == 1 else (hits[0] if hits else None)


def say(text: str = "") -> None:
    """헤드리스 출력. 작업 스케줄러 로그로 그대로 넘어간다."""
    print(f"{datetime.now():%Y-%m-%d %H:%M:%S}  {text}" if text else "", flush=True)


def headless_radar(args) -> int:
    """예매 범위를 한 번(또는 주기적으로) 재서 openlog.jsonl 에 남긴다.

    오픈 예측은 이 기록이 쌓인 만큼 정확해진다. 작업 스케줄러로 10분마다
    --once 를 돌리는 것이 이 모드를 만든 이유다.
    """
    if args.brand not in PROVIDERS:
        say(f"모르는 브랜드: {args.brand} (lotte · megabox · cgv)")
        return 2
    cat = Catalog(PROVIDERS[args.brand]())
    try:
        cinema = find_cinema(cat, args.cinema)
    except (LotteApiError, Unsupported) as exc:
        say(f"지점 목록 실패: {exc}")
        return 1
    if not cinema:
        say(f"지점을 찾지 못했습니다: {args.cinema}")
        return 2

    radar = OpenRadar(cat)
    prev: Horizon | None = None
    for row in OpenRadar.history(cinema.cinema_id, args.movie)[-1:]:
        prev = Horizon(datetime.fromisoformat(row["t"]), cinema.cinema_id, args.movie,
                       row.get("last_open", ""), int(row.get("days", 0)))
    ...
    while True:
        began = time.monotonic()
        try:
            snap = radar.scan(cinema, args.movie)
        except (LotteApiError, Unsupported) as exc:
            say(f"측정 실패: {exc}")
            log_error(f"[headless] {cinema.name}: {exc}")
            snap, rc = None, 1
        if snap and snap.last_open:
            OpenRadar.log(snap)
            head = f"{cat.provider.name} {cinema.name}"
            if args.movie:
                head += f" · {args.movie}"
            if prev and snap.last_open > prev.last_open:
                added = (date.fromisoformat(snap.last_open)
                         - date.fromisoformat(prev.last_open)).days
                say(f"{head} · 예매 오픈! {prev.last_open} -> {snap.last_open} (+{added}일)")
                if args.webhook or args.telegram_token:
                    cfg = Watch(mode="radar", brand=args.brand, movie=args.movie,
                                sound=False, webhook=args.webhook or "",
                                telegram=({"token": args.telegram_token,
                                           "chat_id": args.telegram_chat}
                                          if args.telegram_token else {}))
                    Notifier(cfg).fire(f"[예매 오픈] {args.movie or '전체'} @ {cinema.name}",
                                       f"{prev.last_open} -> {snap.last_open} (+{added}일)")
            else:
                say(f"{head} · D+{snap.days} 까지 (마지막 {snap.last_open}) "
                    f"· API {cat.provider.calls}회")
            prev = snap
        if args.once:
            return rc
        idle = max(args.interval - (time.monotonic() - began), 5)
        time.sleep(idle)


def headless_profile(args) -> int:
    """저장해 둔 감시 설정을 화면 없이 돌린다."""
    rows = [p for p in load_profiles() if p.get("profile_name") == args.profile]
    if not rows:
        names = [p.get("profile_name", "?") for p in load_profiles()]
        say(f"그런 설정이 없습니다: {args.profile}")
        say(f"저장된 것: {', '.join(names) or '(없음)'}")
        return 2
    cfg = cfg_from_dict(rows[0])
    if args.interval:
        cfg.interval = args.interval
    cat = Catalog(PROVIDERS[cfg.brand]())
    st, note = State(cfg), Notifier(cfg)
    worker = {"seat": worker_seat, "open": worker_open, "radar": worker_radar}[cfg.mode]
    say(f"'{cfg.profile_name}' 시작 · {cfg.cinema_name} · {cfg.movie or '전체'}")

    thread = threading.Thread(target=worker, args=(cat, cfg, note, st), daemon=True)
    thread.start()
    seen = 0
    try:
        while thread.is_alive():
            ...
            with st.lock:
                hits = list(st.hits)
            for when, text in reversed(hits[:len(hits) - seen]):
                say("발견  " + ANSI_RE.sub("", text))
            seen = len(hits)
            if args.once and st.cycle >= 1 and not st.prog[1]:
                st.stop.set()
                st.wake.set()
                break
    except KeyboardInterrupt:
        st.stop.set()
        st.wake.set()
    say(f"끝 · {st.cycle}주기 · 발견 {len(st.hits)}건 · API {cat.provider.calls}회")
    return 0


def run_headless(args) -> int:
    UI["tty"] = False                          # 방향키 UI 를 쓰지 않는다
    UI["ansi"] = False
    C.disable()
    load_settings()
    if args.profile:
        return headless_profile(args)
    return headless_radar(args)


def build_parser() -> "argparse.ArgumentParser":
    import argparse
    p = argparse.ArgumentParser(
        prog="cinema_watch.py",
        description="한국 영화관 좌석·예매오픈 감시 콘솔. 인자 없이 실행하면 화면이 뜹니다.",
        epilog="예) python cinema_watch.py --radar --cinema 월드타워 --once")
    p.add_argument("--radar", action="store_true",
                   help="예매 범위를 재서 openlog.jsonl 에 남깁니다 (화면 없이)")
    p.add_argument("--profile", metavar="이름",
                   help="저장해 둔 감시 설정을 화면 없이 돌립니다")
    p.add_argument("--brand", default="lotte", metavar="브랜드",
                   help="lotte · megabox · cgv (기본 lotte)")
    p.add_argument("--cinema", default="", metavar="지점",
                   help="지점 번호 또는 이름 일부 (예: 1016 또는 월드타워)")
    p.add_argument("--movie", default="", metavar="제목", help="영화 제목 일부")
    p.add_argument("--once", action="store_true", help="한 번만 재고 끝냅니다")
    p.add_argument("--interval", type=int, default=600, metavar="초",
                   help="--once 가 아닐 때 반복 주기 (기본 600초)")
    p.add_argument("--webhook", default="", metavar="URL",
                   help="오픈을 잡으면 보낼 Discord/Slack 웹훅")
    p.add_argument("--telegram-token", default="", metavar="토큰")
    p.add_argument("--telegram-chat", default="", metavar="chat_id")
    p.add_argument("--list-cinemas", action="store_true",
                   help="그 브랜드의 지점 번호와 이름을 쭉 찍고 끝냅니다")
    return p


def list_cinemas(brand: str) -> int:
    if brand not in PROVIDERS:
        say(f"모르는 브랜드: {brand}")
        return 2
    cat = Catalog(PROVIDERS[brand]())
    try:
        rows = cat.cinemas()
    except (LotteApiError, Unsupported) as exc:
        say(f"지점 목록 실패: {exc}")
        return 1
    print(f"{PROVIDERS[brand].name} {len(rows)}개 지점")
    for c in sorted(rows, key=lambda x: (x.region_name or "", x.name)):
        print(f"  {str(c.cinema_id):>8}  {c.region_name or '-':10} {c.name}")
    return 0
    ...
    while True:
        try:
            if not menu_once(cat, fav):
                return
        except (BackToMenu, GoBack):       # 첫 단계에서 Esc 를 눌러도 메뉴로 돌아온다
            continue
...
def settings_menu() -> None:
    ...
    while True:
        ...
        cur_tone = next((desc for name, desc in SOUND_CHOICES if name == cur_file),
                        "알람 (기본)")
        try:
            sw = int(SETTINGS.get("seat_width", 0) or 0)
        except (TypeError, ValueError):
            sw = 0
        seat_w_desc = f"{sw}칸" if sw else f"터미널 폭 자동 ({UI['term']}칸)"
        depth = sweet_depth()
        where = ("앞쪽" if depth < 0.45 else "뒤쪽" if depth > 0.75 else "가운데")
        sweet_desc = f"앞에서 {depth:.0%} 지점 ({where})"
        items = [
            ...
            ("seatw", "좌석표 폭", seat_w_desc, ""),
            ("sweet", "명당 기준", sweet_desc, ""),
            ("back", "뒤로", "", C.BYELLOW),
        ]
        ...
        if key == "banner":
            ...
            form("명당 기준", [
                "스크린에서 얼마나 뒤를 좋아하는지 정합니다 (좌우는 늘 정중앙).",
                f"{C.DIM}40 = 앞쪽 · 62 = 기본 · 80 = 뒤쪽{C.RESET}",
            ], ["설정"])
            SETTINGS["sweet_depth"] = ask_int("앞에서 몇 %", int(round(depth * 100)),
                                              10, 95) / 100
        elif key == "seatw":
            form("좌석표 폭", [
                f"터미널이 지금 {UI['term']}칸입니다. 0 을 넣으면 폭에 맞춰 자동으로 씁니다.",
                f"{C.DIM}좁게 고정하고 싶을 때만 숫자를 넣으세요 (48~240).{C.RESET}",
            ], ["설정"])
            SETTINGS["seat_width"] = ask_int("폭 (0=자동)", sw, 0, 240)
        elif key == "tone":
...
def load_history(days: int = 0, brand: str = "") -> list[dict]:
    """history.jsonl 을 읽어 온다. days 를 주면 그만큼만, brand 로 걸러낸다.

    기록에 brand 가 들어간 것은 나중이라, 없는 줄은 브랜드를 안 따진다.
    """
    if not os.path.exists(HISTORY_FILE):
        return []
    cut = (datetime.now() - timedelta(days=days)).isoformat() if days else ""
    rows = []
    try:
        with open(HISTORY_FILE, encoding="utf-8") as fh:
            for line in fh:
                ...
                try:
                    ...
                    continue
                if cut and str(row.get("t", "")) < cut:
                    continue
                if brand and row.get("brand") and row.get("brand") != brand:
                    continue
                rows.append(row)
    except Exception:
        return rows
...
def stats_report(cat: Catalog) -> None:
    """쌓인 관측을 집계해 보여준다 (메뉴 > 감시 통계).

    history.jsonl 은 여태 쌓기만 하고 아무도 읽지 않았다. 잔여석이 늘어난 순간이
    곧 취소이므로, 그 시각을 요일·시간대로 모으면 '언제 지켜봐야 하는지' 가 나온다.
    """
    ranges = [(7, "최근 7일"), (30, "최근 30일"), (0, "전체 기록"), (1, "오늘")]
    days = Chooser("어느 기간을 볼까요", ranges,
                   lambda r: two_col(r[1], "", right_width=8),
                   crumbs=["감시 통계"]).run()[0]
    rows = load_history(days, cat.provider.code)

    clear_screen()
    lines = header("감시 통계", ["감시 통계"])
    if not rows:
        lines.append(box_row(f"{C.YELLOW}아직 쌓인 기록이 없습니다.{C.RESET}"))
        lines.append(box_row(f"{C.DIM}좌석 감시를 돌리면 잔여석 변동이"
                             f" {os.path.basename(HISTORY_FILE)} 에 쌓입니다.{C.RESET}"))
        lines.append(box_bottom())
        print("\n".join(lines))
        read_line(f"   {C.DIM}Enter 로 계속{C.RESET}")
        return

    cancels = [r for r in rows if int(r.get("delta", 0)) > 0]
    sales = [r for r in rows if int(r.get("delta", 0)) < 0]
    gained = sum(int(r["delta"]) for r in cancels)
    span = f"{rows[0].get('t', '')[:10]} ~ {rows[-1].get('t', '')[:10]}"

    lines.append(box_row(f"{C.DIM}{pad('기간', 10)}{C.RESET}{C.BWHITE}{span}{C.RESET}"))
    lines.append(box_row(f"{C.DIM}{pad('관측', 10)}{C.RESET}{len(rows):,}줄   "
                         f"{C.BGREEN}취소 {len(cancels)}회 (+{gained}석){C.RESET}   "
                         f"{C.DIM}판매 {len(sales)}회{C.RESET}"))
    lines.append(box_mid())

    def bars(title: str, counted: Counter, extra: dict, top: int = 6,
             tail: str = "회") -> None:
        lines.append(box_row(f"{C.BOLD}{title}{C.RESET}"))
        if not counted:
            lines.append(box_row(f"  {C.DIM}아직 없음{C.RESET}"))
            return
        peak = counted.most_common(1)[0][1]
        for name, n in counted.most_common(top):
            plus = extra.get(name, 0)
            lines.append(box_row(
                f"  {cell(name, 24)}{bar(n, peak, 16)} {n:>3}{tail}"
                f"{C.DIM}{f' (+{plus}석)' if plus else ''}{C.RESET}"))

    when, when_seats = Counter(), {}
    for r in cancels:
        try:
            t = datetime.fromisoformat(str(r["t"]))
        except ValueError:
            continue
        key = f"{WEEKDAY_KR[t.weekday()]} {t.hour:02d}시"
        when[key] += 1
        when_seats[key] = when_seats.get(key, 0) + int(r["delta"])
    bars("취소가 잦은 때  (관측 시각 기준)", when, when_seats)
    lines.append(box_mid())

    which, which_seats = Counter(), {}
    for r in cancels:
        key = f"{str(r.get('date', ''))[5:]} {r.get('start', '')} {r.get('screen', '')}"
        which[key.strip()] += 1
        which_seats[key.strip()] = which_seats.get(key.strip(), 0) + int(r["delta"])
    bars("취소가 잦은 회차", which, which_seats, top=5)
    lines.append(box_mid())

    what = Counter(f"{r.get('cinema_name') or r.get('cinema')} · {r.get('movie', '')}"
                   for r in rows)
    bars("가장 많이 관측한 대상", what, {}, top=4, tail="줄")
    lines.append(box_bottom())
    print("\n".join(lines))
    print(f"   {C.DIM}취소가 잦은 때를 보고 감시 시간을 정하면 헛도는 주기가"
          f" 줄어듭니다.{C.RESET}")
    read_line(f"   {C.DIM}Enter 로 계속{C.RESET}")
...
def edit_profile(prof: dict) -> None:
    """저장된 설정의 이름을 바꾸거나 지운다 (저장 목록에서 e)."""
    name = str(prof.get("profile_name") or "(이름없음)")
    show_cursor()
    clear_screen()
    print("\n".join(header(f"설정 편집 - {name}", ["저장된 설정"])))
    print(box_row(f"{C.DIM}{pad('극장', 10)}{C.RESET}{prof.get('cinema_name', '-')}"))
    print(box_row(f"{C.DIM}{pad('영화', 10)}{C.RESET}{prof.get('movie') or '전체'}"))
    print(box_bottom())

    what = Chooser("무엇을 할까요", [
        ("rename", "이름 바꾸기", ""),
        ("delete", "삭제", "되돌릴 수 없습니다"),
        ("back", "그대로 두기", ""),
    ], lambda it: two_col(it[1], it[2], right_width=22,
                          color=C.BRED if it[0] == "delete" else ""),
        crumbs=["저장된 설정", trim(name, 14)]).run()[0]

    if what == "rename":
        new = ask_text("새 이름", name).strip()
        if new and new != name:
            if rename_profile(name, new):
                print(f"   {C.BGREEN}이름을 바꿨습니다.{C.RESET}")
            read_line(f"   {C.DIM}Enter 로 계속{C.RESET}")
    elif what == "delete":
        if ask_yes(f"'{trim(name, 20)}' 을(를) 지울까요?", False):
            if delete_profile(name):
                print(f"   {C.BGREEN}지웠습니다.{C.RESET}")
            read_line(f"   {C.DIM}Enter 로 계속{C.RESET}")
    hide_cursor()
...
def menu_once(cat: Catalog, fav: Favorites) -> bool:
    ...
    if os.path.exists(HISTORY_FILE):
        menu.append(("stats", "감시 통계", "언제·어느 회차에 취소가 잦았는지", main_c))
    ...
    if picked == "fav":
        ...
        prof = Chooser("저장된 설정", profiles,
                       lambda p: two_col(p.get("profile_name", "(이름없음)"),
                                         ...
                                         f"{p.get('movie') or '전체'}",
                                         right_width=32),
                       hint="Enter 로 시작 · e 로 이름변경·삭제",
                       preview=("e", lambda p: edit_profile(p)),
                       preview_label="이름변경·삭제").run()
        if any(p.get("profile_name") == prof.get("profile_name")
               for p in load_profiles(brand.code)):
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 1:               # 인자가 있으면 화면 없이 돈다
            _args = build_parser().parse_args()
            if _args.list_cinemas:
                raise SystemExit(list_cinemas(_args.brand))
            if not (_args.radar or _args.profile):
                build_parser().print_help()
                raise SystemExit(2)
            raise SystemExit(run_headless(_args))
        main()
    except KeyboardInterrupt:
        show_cursor()
        print("\n   종료합니다.")
